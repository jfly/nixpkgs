...
let
  ...
  # This is a implemenation of a simple libconfuse config renderer sufficient
  ...
  configFile = pkgs.writeText "postsrsd.conf" (renderAttr cfg.settings);
in
{
  imports =
    [
      (mkRemovedOptionModule [ "services" "postsrsd" "socketPath" ] ''
        Configure/reference `services.postsrsd.settings.socketmap` instead. Note that it has to start with the `inet:` or `unix:` prefix.
      '')
      (mkRenamedOptionModule
        ...
        type = lib.types.submodule {
          ...
          options = {
            domains = lib.mkOption {
              ...
              example = [ "example.com" ];
              description = ''
                List of local domains that dont't require rewriting
              '';
            };
            ...
            separator = lib.mkOption {
              ...
              default = "=";
              description = ''
                Separator character in generated addresses
              '';
            };
            ...
            srs-domain = lib.mkOption {
              ...
              description = ''
                ...
                It is recommended to configure a dedicated SRS rewrite domain
                to prevent privacy issues when serving multiple unrelated
                domains.

                ::: {.warning}
                For privacy reasons the domain used for SRS should not be used as a regular mail domain!
                :::
              '';
            };
            ...
            chroot-dir = lib.mkOption {
              ...
              description = ''
                ...
                ::: {.note}
                We confine the runtime environment throught systemd hardening instead, so this option is read-only.
                :::
              '';
            };
  ...
  config = lib.mkMerge [
    (lib.mkIf (cfg.enable && cfg.configurePostfix && config.services.postfix.enable) {
      services.postfix.config = lib.mkIf (cfg.configurePostfix && config.services.postfix.enable) {
      ...
      systemd.services.postfix.serviceConfig.SupplementaryGroups = [ cfg.group ];
    })
