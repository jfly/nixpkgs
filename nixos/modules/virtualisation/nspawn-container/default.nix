  ...
  config = {
    ...
    system.build.nspawn =
      pkgs.writers.writePython3Bin "run-${config.system.name}-nspawn"
        ...
        ''
          ${builtins.readFile ./run_nspawn.py}

          run_nspawn(
            container_name=${builtins.toJSON config.system.name},  # noqa
            root_dir_str=${builtins.toJSON cfg.rootDir},  # noqa
            interfaces_json=${builtins.toJSON (builtins.toJSON (lib.attrValues cfg.allInterfaces))},  # noqa
            nspawn_options=${builtins.toJSON config.virtualisation.systemd-nspawn.options},  # noqa
            init=${builtins.toJSON "${config.system.build.toplevel}/init"},  # noqa
            init_args=${builtins.toJSON cfg.initArgs},  # noqa
          )
        '';
  };
}
