...
let
  ...
  vmNodeHostNames =
    ...
    in
    nodesList ++ lib.optional (exactlyOneMachine && !lib.elem "machine" nodesList) "machine";

  containerNodeHostNames =
    let
      nodesList = map (c: c.system.name) (lib.attrValues config.containers);
    in
    nodesList ++ lib.optional (exactlyOneMachine && !lib.elem "machine" nodesList) "machine";
  ...
  pythonizedVmNames = map pythonizeName vmNodeHostNames;
  vmMachineNames = map (name: "${name}: QemuMachine;") pythonizedVmNames;

  pythonizedContainerNames = map pythonizeName containerNodeHostNames;
  containerMachineNames = map (name: "${name}: NspawnMachine;") pythonizedContainerNames;
    ...
    hostPkgs.runCommand "nixos-test-driver-${config.name}"
      ...
      ''
        ...
        ${lib.optionalString (!config.skipTypeCheck) ''
          ...
          echo "${builtins.toString vmMachineNames}" >> testScriptWithTypes
          echo "${builtins.toString containerMachineNames}" >> testScriptWithTypes
